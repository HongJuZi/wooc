...
var HHTask    = function(cfg) {
    ...
    this.logTpl  = '<div class="alert alert-block alert-{type}">'
        ...
        + ' </div>';
    this.task   = null;
    this.run   = function(url, formData, succeedCallback, failCallback) {
        this.task     = {
            url: url, 
            formData: formData, 
            callback: {succeed: succeedCallback, fail: failCallback}
        };
        this.start();
        this.bindStop();
        this.hold();
                ...
                $.post(
                    ...
                    function(response) {
                        ...
                        if(false === response.rs) {
                            ...
                            _root.execCallback(response, 'fail');
                            return;
                        }
                        _root.times ++;
                        _root.execCallback(response.data, 'succeed');
                        _root.info(
                            "Hi，亲刚才安全地完成了第“"
    ...
    this.execCallback   = function(response, type) {
        if(null === this.task) {
            return;
        }
        if('undefined' === typeof(this.task.callback[type])) {
            return;
        }
        this.task.callback[type](response, this);
    };
